...
/**
 * This is the model class for table "seguridad.t_acceso".
 *
 * The followings are the available columns in table 'seguridad.t_acceso':
...
class TAcceso extends CActiveRecord
{
	...
	public function verificarIntentos($id,$acceso)	
	{
		...
		if(!empty($access)){
			...
			if (($intento==3)||($val==3)){
				...
				$sql = "UPDATE seguridad.t_usuario SET id_estatus=:valor, nro_intentos=:intentos  WHERE id_usuario =:id";
				...
				$intento=$access->nro_intentos;
				$val= $intento+1;
				$sql = "UPDATE seguridad.t_usuario SET nro_intentos=:valor  WHERE id_usuario =:id";
				$comando = Yii::app() -> db -> createCommand($sql);
				$comando -> bindParam(":id",$idUsuario, PDO::PARAM_INT); 
	...
	public function verificarSesionMultip($id)
	{
		...
		$criteria->condition = "t.id_usuario = ".$id." and importancia = (Select max(importancia) from seguridad.t_usuario_rol)";
        ...
        if(!empty($rol_usuario)){
			...
			if(!empty($acceso_anterior)){ // 2 acceso con la misma ip
				if ($acceso_anterior->nro_sesion==0){
				    $v=1;
				    $acceso="exitoso";
				    $sql = "UPDATE seguridad.t_acceso SET nro_sesion=:intentos, fecha_ingreso_usuario=now(), acceso=:access  WHERE id_usuario =:id and ip_address=:ips";
					$comando = Yii::app() -> db -> createCommand($sql);
				   ...
				   // $comando -> bindParam(":now",now(), PDO::PARAM_TIME);
					...
					$intentos_access=TUsuario::model()->findByAttributes(array('id_usuario'=>$id));
						if($intentos_access->nro_intentos!=0){
				      		    $intent=0;
				      			$sql = "UPDATE seguridad.t_usuario SET nro_intentos=:intentos WHERE id_usuario =:id";
				    			$comando = Yii::app() -> db -> createCommand($sql);
				        		$comando -> bindParam(":id",$id, PDO::PARAM_INT);
				    ...
				    if($boolSesion==1){
				                ...
				                $sql = "UPDATE seguridad.t_acceso SET nro_sesion=:intentos, fecha_ingreso= now()  WHERE id_usuario =:id and ip_address=:ips";
								$comando = Yii::app() -> db -> createCommand($sql);
								$comando -> bindParam(":id",$id, PDO::PARAM_INT);
				                //$comando -> bindParam(":now",now(), PDO::PARAM_TIME);
								$comando -> bindParam(":intentos",$v, PDO::PARAM_INT);
		...
		//echo "id".$id."ip".$ip;
		 ...
		 if(!empty($eliminarAcceso)){
		 	 ...
		 	 if($n_sesion>1){
                ...
                $sql = "UPDATE seguridad.t_acceso SET nro_sesion=:intentos, fecha_ingreso= now()  WHERE id_usuario =:id and ip_address=:ip";
                $comando = Yii::app() -> db -> createCommand($sql);
                $comando -> bindParam(":id",$id, PDO::PARAM_INT);               
                ...
                if( $control){
		 	 	...
		 	 	$sql = "DELETE from seguridad.t_acceso WHERE id_usuario =:id and ip_address=:ip";
                $comando = Yii::app() -> db -> createCommand($sql);
                $comando -> bindParam(":id",$id, PDO::PARAM_INT);
                $comando -> bindParam(":ip",$ip, PDO::PARAM_STR);
                $control = $comando -> execute();
